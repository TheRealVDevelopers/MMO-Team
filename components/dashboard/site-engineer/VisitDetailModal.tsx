            ...
            <div className="space-y-4">
                ...
                <div className="p-4 bg-subtle-background rounded-lg space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <button onClick={() => handleStatusUpdate(SiteVisitStatus.COMPLETED)} disabled={!visit.onSiteTime || !!visit.completionTime} className="p-2 text-sm font-semibold rounded-md disabled:opacity-50 text-white bg-secondary disabled:bg-secondary/50">Mark as Complete</button>
                        <button onClick={handleSubmit} disabled={!visit.completionTime || visit.status === SiteVisitStatus.REPORT_SUBMITTED} className="p-2 text-sm font-semibold rounded-md disabled:opacity-50 text-white bg-secondary disabled:bg-secondary/50">Submit Report</button>
                    </div>
                    ...
                    {activeTab === 'report' && (
                        <div className="space-y-4">
                            ...
                            <h3 className="font-bold pt-2">Measurements <span className="text-xs font-normal text-text-secondary">(Total Area: {totalArea.toFixed(2)} sq ft)</span></h3>
                             ...
                             <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="Add final notes or summary..." className="mt-2 w-full p-2 border-border bg-surface rounded-md" />
                        </div>
                    )}
