...
const RequestStatusPill: React.FC<{ status: ApprovalStatus }> = ({ status }) => {
  const config = {
    ...
    [ApprovalStatus.AWAITING_EXECUTION_ACCEPTANCE]: { color: 'text-blue-500 bg-blue-500/10', icon: ClockIcon, label: 'Awaiting Acceptance' },
    [ApprovalStatus.NEGOTIATION]: { color: 'text-purple-500 bg-purple-500/10', icon: ChatBubbleLeftRightIcon, label: 'Negotiation' },
  }[status] || { color: 'text-gray-500 bg-gray-500/10', icon: ClockIcon, label: status };
