...
const SiteVisitTasksPage: React.FC<{ setCurrentPage: (page: string) => void; siteVisits: SiteVisit[]; onScheduleVisit: (visitData: Omit<SiteVisit, 'id' | 'status'>) => void; }> = ({ setCurrentPage, siteVisits, onScheduleVisit }) => {
                    ...
                    <table className="min-w-full divide-y divide-border">
                        ...
                        <tbody className="bg-surface divide-y divide-border">
                            {myVisits.map(visit => (
                             ...
                             {myVisits.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="text-center py-8 text-text-secondary">No site visits assigned yet.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
