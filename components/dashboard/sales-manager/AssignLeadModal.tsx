...
const AssignLeadModal: React.FC<AssignLeadModalProps> = ({ isOpen, onClose, leads, users, onAssignLead }) => {
  ...
  const { getNextAvailableTeamMember, loading: assignmentLoading } = useSmartAssignment();
  ...
  const handleAutoAssign = () => {
    const recommendedMember = getNextAvailableTeamMember();
    if (recommendedMember) {
      setSelectedRepId(recommendedMember.userId);
    } else {
        ...
        <div className="flex flex-col border border-border rounded-lg bg-subtle-background">
          <div className="p-3 border-b border-border flex justify-between items-center">
            ...
            <button
              ...
              className="text-xs flex items-center gap-1.5 px-2 py-1 rounded bg-primary/10 text-primary hover:bg-primary hover:text-white font-bold uppercase tracking-wide transition-all disabled:opacity-50"
              title="Smart Auto-Assign based on availability"
            >
