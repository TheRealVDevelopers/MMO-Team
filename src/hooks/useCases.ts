...
export const useCases = (options: UseCasesOptions = {}) => {
  ...
  useEffect(() => {
    if (!db) {
      console.error('[useCases] Firebase database not initialized');
    ...
    if (!options.organizationId) {
      console.warn('[useCases] No organizationId provided, returning empty data');
      setCases([]);
      setLoading(false);
      return;
    }

    console.log('[useCases] Setting up listener with options:', options);
    ...
    try {
      ...
      console.log('[useCases] Collection path:', `${FIRESTORE_COLLECTIONS.ORGANIZATIONS}/${options.organizationId}/${FIRESTORE_COLLECTIONS.CASES}`);
      ...
      if (options.isProject !== undefined) {
        ...
        console.log('[useCases] Filtering by isProject:', options.isProject);
      }
      ...
      if (options.status) {
        ...
        if (statuses.length === 1) {
          ...
          console.log('[useCases] Filtering by status:', statuses[0]);
        }
      ...
      if (options.userId) {
        console.log('[useCases] Will filter by userId client-side:', options.userId);
      ...
      const unsubscribe = onSnapshot(
        q,
        (snapshot) => {
          console.log('[useCases] Received snapshot with', snapshot.docs.length, 'documents');
          ...
          if (options.userId) {
            const beforeFilter = casesData.length;
            casesData = casesData.filter(
              (c) => c.assignedSales === options.userId || c.projectHead === options.userId
            );
            console.log('[useCases] Filtered by userId:', beforeFilter, '->', casesData.length);
          }
          ...
          if (Array.isArray(options.status) && options.status.length > 1) {
            ...
            console.log('[useCases] Filtered by multiple statuses:', beforeFilter, '->', casesData.length);
          }

          console.log('[useCases] Final filtered cases:', casesData.length);
          setCases(casesData);
          setLoading(false);
          setError(null);
        },
        (err) => {
          ...
          console.error('[useCases] Error details:', {
            code: err.code,
            message: err.message,
      ...
      return () => {
        console.log('[useCases] Cleaning up listener');
        unsubscribe();
      };
    } catch (err: any) {
      console.error('[useCases] Error setting up cases listener:', err);
      setError(err.message);
      setLoading(false);
    }
