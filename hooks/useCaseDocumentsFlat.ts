...
import { uploadCaseDocuments, deleteFile } from '../services/storageService';

interface UseCaseDocumentsFlatOptions {
  caseId: string;
  type?: DocumentType | DocumentType[];
}

export const useCaseDocumentsFlat = (options: UseCaseDocumentsFlatOptions) => {
  ...
  useEffect(() => {
    ...
    try {
      // FLAT STRUCTURE: cases/{caseId}/documents
  ...
  const uploadFileWithStorage = useCallback(
    async (
      ...
      try {
        ...
        // 2. Save metadata to Firestore (FLAT STRUCTURE)
        const documentsRef = collection(
          db,
          FIRESTORE_COLLECTIONS.CASES,
          options.caseId,
